...
class NST:
    ...
    def variational_cost(generated_image):
        ...
        """
        x_diff = generated_image[:, :, 1:, :] - \
            generated_image[:, :, :-1, :]

        y_diff = generated_image[:, 1:, :, :] - \
            generated_image[:, :-1, :, :]

        return tf.reduce_sum(tf.abs(x_diff)) + \
            tf.reduce_sum(tf.abs(y_diff))
